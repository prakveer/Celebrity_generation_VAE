...
ndf = 128# parameter in determining encoder channels (see encoder)
nc = 1# o/p of decoder: one channel for our case
# nz: latent dimension
nz = 128
lr = 0.001
beta1=0.5# parameter in adam optimiser
...
class VAE(object):
    ...
    def loss(self):
            with tf.variable_scope('losses', reuse=tf.AUTO_REUSE) as scope:
                ...
                self.loss = self.loss_recon + 5 * 1e-4 * self.KL
    ...
    def train_one_epoch(self, sess, saver, writer, epoch, step):
        ...
        for i in range (int( (1*len(self.true_imgs_dataset) )/ self.batch_size)):
            ...
            # reconstructed image and random generated images
